...
│   └── imgly/
│       ├── index.ts          # Editor initialization with background removal
│       ├── plugins/
│       │   └── background-removal.ts  # Background removal plugin (Apps dock + canvas menu)
│       └── config/
...
│           └── ui/               # UI layout configuration (dock includes Apps)
├── public/
│   └── assets/
│       └── remove-bg.png         # Background removal thumbnail
├── package.json
└── vite.config.ts
```
